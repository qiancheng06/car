...
public:
  ...
  : rclcpp::Node("car_test"),
    pid_(),
    speed_(declare_parameter<double>("speed", 1535.0)),
    max_left_distance_(declare_parameter<double>("max_left_distance", 1.5)),
...
private:
  ...
  void laserCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
  {
    ...
    auto filterOutliers = [](std::array<BucketPoint, 30U> &points, int &count)
    {
      if (count <= 1)
      {
        return;
      ...
      const double limit = 1.75 * std::max(std::abs(avg), 0.05);

      int write = 0;
      for (int i = 0; i < count; ++i)
      {
        if (std::abs(points[i].x - avg) <= limit)
        {
    ...
    filterOutliers(red_points, red_count);
    filterOutliers(blue_points, blue_count);
    ...
    if (error_count > 0)
    {
      ...
      const double error = weight_sum > 0.0 ? (weighted_error / weight_sum) * 20.0 : 0.0;
