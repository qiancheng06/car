...
DISTANCE_THRESHOLDS = {
    "first": 5.4,        # 第一模式触发阈值
    #"second": 6.0,       # 第二模式触发阈值
    "second": 12.5,       # 第二模式触发阈值
    #"third": 15.0,       # 第三模式触发阈值
    "third": 24.7,       # 第三模式触发阈值
    #"fourth": 17.0,      # 第四模式触发阈值
    "fourth": 25.7,      # 第四模式触发阈值
    #"fifth": 18.5,       # 第五模式触发阈值
    "fifth": 27.65,       # 第五模式触发阈值
    #"sixth": 28.0,       # 第六模式触发阈值
    "sixth": 29.5,       # 第六模式触发阈值
    #"lap_total": 35.0,   # 单圈总距离（判断第二圈）
    "lap_total": 35.0,   # 单圈总距离（判断第二圈）
    "traffic_light": 21.3, # 红绿灯距离触发阈值
    "ab_stop": 33.0      # 第二圈AB停车触发距离（可根据实际调整）
}
...
class LidarCoordinateControlNode(Node):
    def __init__(self):
        ...
        self.special_linear_speed1 = 0.4
        self.special_angular_speed1 = 95.7
        self.special_angular_speed3 = 97.5
        #self.special_angular_speed3 = 70.0
        self.special_linear_speed5 = 0.4
        self.special_angular_speed5 = 96.3
        #self.special_angular_speed5 = 70.0
        self.special_linear_speed4 = 0.4
        ...
        #self.ab_fixed_angle = 80.0  # AB模式固定角度（可根据需求调整）
        self.ab_target_distance = 0.7  # AB模式需行驶的距离（1米）
        ...
        self.lap_proximity_threshold = 0.55  # 判定回到起点的距离阈值（米）
...
def main(args=None):
    ...
    try:
        ...
        # 手动中断时强制停车
        stop_cmd = Twist()
        stop_cmd.linear.x = node.stop_brake_speed
