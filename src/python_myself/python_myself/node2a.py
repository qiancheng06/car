...
DISTANCE_THRESHOLDS = {
    ...
    "second": 15,       # 第二模式触发阈值
    #"third": 15.0,       # 第三模式触发阈值
    "third": 24.75,       # 第三模式触发阈值
    #"fourth": 17.0,      # 第四模式触发阈值
    "fourth": 25.75,      # 第四模式触发阈值
    #"fifth": 18.5,       # 第五模式触发阈值
    "fifth": 27.45,       # 第五模式触发阈值
    #"sixth": 20.0,       # 第六模式触发阈值
    "sixth": 29.25,       # 第六模式触发阈值
    #"lap_total": 35.0,   # 单圈总距离（判断第二圈）
    "lap_total": 35.8,   # 单圈总距离（判断第二圈）
    "traffic_light": 22.0, # 红绿灯距离触发阈值
    "ab_stop": 36.0      # 第二圈AB停车触发距离（可根据实际调整）
}
...
class LidarCoordinateControlNode(Node):
    def __init__(self):
        ...
        self.special_angular_speed5 = 97.5
        ...
        self.stop_brake_speed = -5.0  # 停车时给更大的反向速度用于制动，加快刹车
        # AB模式参数（新增）
        ...
        self.lap_proximity_threshold = 0.35  # 判定回到起点的距离阈值（米）
        self.lap_proximity_hits = 0      # 累计满足阈值的连续计数
        self.lap_proximity_required = 1  # 连续命中次数，避免一次抖动误判
        ...
        # 注册关闭钩子，确保 Ctrl+C 等退出时先发停车指令（兼容无 on_shutdown 的 rclpy）
        try:
            ctx = rclpy.get_default_context()
            if hasattr(ctx, "on_shutdown"):
                ctx.on_shutdown(self.on_shutdown_hook)
            else:
                self.get_logger().warn("当前 rclpy 未提供 on_shutdown，Ctrl+C 仅依赖 KeyboardInterrupt 处理停车")
        except Exception as exc:
            self.get_logger().warn(f"注册关闭钩子失败: {exc}")

        self.get_logger().info("激光雷达距离控制节点已启动（距离触发模式）")

    def apply_stop(self, brake=True):
        brake_speed = self.stop_brake_speed if brake else 0.0
        self.cmd_vel.linear.x = brake_speed
        self.cmd_vel.angular.z = python_angle_to_angular_z(base_angle)

    def publish_force_stop(self, repeats=3, delay=0.05):
        # 多次发布停车指令以提高 Ctrl+C 退出时生效的概率
        self.apply_stop(brake=True)
